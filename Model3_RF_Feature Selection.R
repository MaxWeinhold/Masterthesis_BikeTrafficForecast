...
#Maximilian Weinhold
#------------------------------------------------------------------------
#Model calculations: Random Forest Modell_Feature Selection
...
for(i in 1:length(validation_set)){
  ...
  model1 <- randomForest(log(Value) ~ Year + Months + Hour + Weekend + Night + publicHoliday + schoolHoliday, data =  train.data, ntree=100, importance=TRUE)
  ...
  Evaluation_DF1$Time[i] = difftime(loop_end_time1, loop_start_time1, units='mins')
  
  beep("coin")
  
  loop_start_time2 <- Sys.time()
  model1 <- randomForest(log(Value) ~ Year + Months + Hour + Weekend + Night + publicHoliday + schoolHoliday +
                 Wind + CloudCover + Humidity + Rain + Temperature, data =  train.data, ntree=100, importance=TRUE)
  ...
  model1 <- randomForest(log(Value) ~ Year + Months + Hour + Weekend + Night + publicHoliday + schoolHoliday +
                 ...
                 young18 + young25 + older40 + older60 + Immigrants + PKWs, data =  train.data, ntree=100, importance=TRUE)
  ...
  model1 <- randomForest(log(Value) ~ Year + Months + Hour + Weekend + Night + publicHoliday + schoolHoliday +
                 ...
                 ClothesShop500mmRadius2 + ClosestTrainS2 + ClosestBridge2 + young302 + PKWs2, data =  train.data, ntree=100, importance=TRUE)
  ...
  model1 <- randomForest(log(Value) ~ Year + Months + Hour + Weekend + Night + publicHoliday + schoolHoliday +
                 ...
                 Rain3 + Inhabitants3 + UniBuild500mmRadius3 + ClothesShop500mmRadius3 + ClosestTrainS3, data =  train.data, ntree=100, importance=TRUE)
...
png(file="plot40.png",width=800, height=800)
R2_plot
dev.off()

png(file="plot41.png",width=800, height=800)
R2_plot_time
dev.off()

write.csv(TrainR,"Plot40_41_Train_Data.csv")
write.csv(TestR,"Plot40_41_Test_Data.csv")

png(file="plot42.png",width=800, height=800)
MSE_plot
dev.off()

png(file="plot43.png",width=800, height=800)
MSE_plot_time
dev.off()

write.csv(TrainR,"Plot42_43_Train_Data.csv")
write.csv(TestR,"Plot42_43_Test_Data.csv")
