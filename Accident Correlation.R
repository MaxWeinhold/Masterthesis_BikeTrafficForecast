...
#Maximilian Weinhold
#------------------------------------------------------------------------
#Data preperation: Accident Correlation

#The sf we will need to make geometrical calculations.
if(!require("sf")) install.packages("sf")
library(sf)

#package for calculating distance using longitude and latitude
...
mapData$Accidents8 = 0
mapData$Accidents16 = 0
mapData$Accidents32 = 0
mapData$Accidents64 = 0
mapData$Accidents128 = 0
mapData$Accidents256 = 0
mapData$Accidents512 = 0
mapData$Accidents1024 = 0

i=1
for(i in 1:nrow(AccidentData)){
  
  start_time <- Sys.time()
  print(paste("Progress is",i/nrow(AccidentData)*100,"%"))
  
  dist = c(1:nrow(mapData))
  dist = as.data.frame(dist)
  names(dist)[1] = "Dist"
  dist$Dist = 0
  
  for(j in 1:nrow(mapData)){
    
    dist = distm(c(AccidentData$longitude[i],AccidentData$latitude[i]), 
          c(mapData$Lon[j],mapData$Lat[j]), 
          fun=distGeo)
    
    if(dist<8){mapData$Accidents8[j] = mapData$Accidents8[j]+1}
    if(dist<16){mapData$Accidents16[j] = mapData$Accidents16[j]+1}
    if(dist<32){mapData$Accidents32[j] = mapData$Accidents32[j]+1}
    if(dist<64){mapData$Accidents64[j] = mapData$Accidents64[j]+1}
    if(dist<128){mapData$Accidents128[j] = mapData$Accidents128[j]+1}
    if(dist<256){mapData$Accidents256[j] = mapData$Accidents256[j]+1}
    if(dist<512){mapData$Accidents512[j] = mapData$Accidents512[j]+1}
    if(dist<1012){mapData$Accidents1012[j] = mapData$Accidents1012[j]+1}
    
  }
  
  end_time <- Sys.time()
  difference <- difftime(end_time, start_time, units='mins')
  duration = as.numeric(difference) *(nrow(AccidentData)-i)/60
  print(paste("The process will go on for",duration,"hours"))
}

levels(as.factor(mapData$Accidents))
...
#Correlation between Bike Traffic and Bike Accidents
