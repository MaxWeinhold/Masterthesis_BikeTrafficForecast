...
if(!require("SpatialML")) install.packages("SpatialML")
library(beepr)
...
for(i in 1:length(validation_set)){
  ...
  train.data$Town = NULL
  names(train.data)
  length(names(train.data))
  train.data$x1 = 1
  train.data$x2 = 2
  Coords<-train.data[ ,144:145]
  ...
  print(start_time)
  model <- grf(Value ~ Year + Months + Hour + Weekend + Night + publicHoliday + schoolHoliday +
                 ...
                 Rain2 + Temperature2 + Inhabitants2 + stre_lengths2, dframe = train.data,
                 bw = 3, kernel="fixed", ntree=50, importance="impurity", coords = Coords)
